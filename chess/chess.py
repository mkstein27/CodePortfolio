    ...
    def moves(self, board):
        for key in board.get_keys():
            ...
            if row == self.pos[0] and col == self.pos[-1]:
                board.set(key,Rook.get_name(Rook))
                continue
            elif row == self.pos[0]:
                board.set(key,'x')
            elif col == self.pos[-1]:
...
class Bishop(Chess_Piece):
    ...
    def moves(self, board):
        for key in board.get_keys():
            row, col = self.get_index(key)
            if row == self.pos[0] and col == self.pos[-1]:
                board.set(key,Bishop.get_name(Bishop))
                continue
...
class King(Chess_Piece):
    ...
    def moves(self,board):
        for key in board.get_keys():
            row, col = self.get_index(key)
            if row == self.pos[0] and col == self.pos[-1]:
                board.set(key,King.get_name(King))
...
class Queen(Chess_Piece):
    ...
    def moves(self,board):
        for key in board.get_keys():
            row, col = self.get_index(key)
            if row == self.pos[0] and col == self.pos[-1]:
                board.set(key,Queen.get_name(Queen))
                continue
            elif abs(row - self.pos[0]) == abs(col - self.pos[1]):
                board.set(key, 'x')
            elif row == self.pos[0]:
